...
function getLocalIP(): string {
  const interfaces = os.networkInterfaces(); // Obtiene las interfaces de red
  for (const name of Object.keys(interfaces)) {
    for (const net of interfaces[name]!) {
      if (net.family === 'IPv4' && !net.internal) { // Filtra las interfaces IPv4 no internas
        return net.address; // Devuelve la IP local
      }
    }
  }
  return 'localhost'; // Si no se encuentra ninguna, devuelve 'localhost'
}
...
http.createServer(app).listen(HTTP_PORT, '0.0.0.0', () => {
  const url = `http://${localIP}:${HTTP_PORT}`; // Usar IP local para el servidor HTTP
  ...
  qrcode.generate(url, { small: true }); // Generar QR del enlace
  qrcode.generate(localhostUrl, { small: true }); // Generar QR del localhost
});
...
try {
  ...
  if (fs.existsSync(keyPath) && fs.existsSync(certPath)) {
    ...
    const httpsUrl = `https://${localIP}:${HTTPS_PORT}`; // Usar IP local para el servidor HTTPS
    ...
    https.createServer({ key, cert }, app).listen(HTTPS_PORT, '0.0.0.0', () => {
      ...
      qrcode.generate(httpsUrl, { small: true }); // Generar QR del enlace HTTPS
      qrcode.generate(httpsLocalhostUrl, { small: true }); // Generar QR del localhost HTTPS
    });
  } else {
